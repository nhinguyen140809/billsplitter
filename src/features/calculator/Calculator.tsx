...
function Calculator({
  ...
  const { expression, errorMessage, handleClick, handleSave } = useCalculator(onClose, onSave)
  ...
  return (
    <Overlay>
      <Popup title="Calculator" className="w-full max-w-md min-w-75">
        <CalulatorDisplay expression={expression} errorMessage={errorMessage} />
        <div className="mt-2 mb-4 grid grid-cols-4 gap-3 gap-x-4">
